...
import {BottomSheetModalComponent} from '../bottom-sheet-menu/bottom-sheet-modal/bottom-sheet-modal-component';
...
import {useBottomSheetMenu} from '../bottom-sheet-menu/bottom-sheet-menu-context';
...
export const AddCommentBottomSheetMenu = () => {
  ...
  const onChangeModal = (index: number) => {
    if (index === -1) {
      setRating(0);
      setIsVisibleCheckBoxVariants(false);
  ...
  return (
    <BottomSheetModalComponent
      ...
      snapPointsProps={snapPoints}>
      <ScrollView contentContainerStyle={{paddingBottom: 50}}>
        <View style={styles.container}>
          <Text style={styles.title}>Оцените заказ</Text>
          {isVisibleCheckboxVariants ? (
            <Text style={styles.description}>
              Вы поставили {getRatingCountText(rating)} звезды. Что вам не понравилось?
            </Text>
          ) : (
            <Text style={styles.description}>
